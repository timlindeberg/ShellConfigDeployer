...
class HostCommunicator:
    ...
    def send_file(self, file_from, file_to):
        def _send_file(connection):
            ...
            self.printer.info("Deploying file %s to %s:%s", file_from, self.host, file_to, verbose=True)
        ...
        try:
            ...
            if self.password is None:
                self.printer.error(
                    ...
                    "Provide a password using the %s, %s or %s flags.", self.host, "-r", "-f", "-p"
                )
            else:
                self.printer.error("Permission denied.")
            sys.exit(5)
        except Exception as e:
            self.printer.error("Could not connect to %s", self.host)
